...
From this initial analysis, we find that the mean total steps per day is `r format(round(stp_mean * 1000), big.mark = ",")` steps and the median is `r format(round(stp_median * 1000), big.mark = ",")` steps.
...
Our histogram now looks much closer to being normally distributed, and, in fact, our mean and median are very close. The mean total steps taken each day is `r format(round(stp2_mean * 1000), big.mark = ",")` steps and the median is `r format(round(stp2_median *1000), big.mark=",")` steps.
...
The time series plot we obtain appears to match a typical diurnal schedule with minimal activity between 11:00pm and 5:00am. The peak activity occurs at `r peak_time_formatted`. Perhaps, this indicates a daily morning exercise.
...
The mean of our new, imputed data is `r format(round(stp3_mean * 1000), big.mark = ",")` steps and the median is `r format(round(stp3_median *1000), big.mark = ",")` steps. Which is nearly the same result we achieved by simply removing the missing days. This makes sense, since we replaced all the zero values from our first histogram with the mean value of our second histogram. 
